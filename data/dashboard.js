...
(function () {
    ...
    function activateSection(slug) {
        navItems.forEach(el => el.classList.toggle('active', el.dataset.section === slug));
        panels.forEach(p => p.classList.toggle('active', p.id === 'section-' + slug));
    ...
    const initSlug = location.hash.slice(1) || (navItems[0] ? navItems[0].dataset.section : 'overview');
    ...
    function loadSection(slug) {
        ...
        if (!source) { loaded[slug] = true; return; }

        const container = panel.querySelector('.stats-grid');
        if (container) {
            container.innerHTML = '<div class="stat-card" style="opacity:.4"><div class="stat-label">Loading…</div><div class="stat-value">—</div></div>';
        }
        ...
        fetch('/api/section.php?source=' + encodeURIComponent(source))
            ...
            .then(function(data) { renderSection(panel, data); })
            .catch(function(err) {
                if (container) {
                    container.innerHTML = '<div class="alert error">⚠ Could not load data: ' + err.message + '</div>';
                }
            })
            .finally(function() { loaded[slug] = true; });
    }
    ...
    function renderSection(panel, data) {
        ...
        grid.innerHTML = cards || '<p style="color:var(--muted)">No data returned.</p>';

        const badge = panel.querySelector('.badge-status');
        if (badge) { badge.className = 'badge-status live'; badge.textContent = 'Live'; }
    }
    ...
    function updateClock() {
        ...
        el.textContent = days[now.getDay()] + ', ' + pad(now.getDate()) + ' ' +
